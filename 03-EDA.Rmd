...
```{r}
tabla_univariada(encuesta, p8, desertor)
```
```{r}
encuesta
```
...
```{r}
encuesta %>% select(p12_1:p12_10ts) %>% filter(p12_8 == " Si")

tabla_univariada(encuesta, p12_8, desertor)


tabla_univariada(encuesta, p32, desertor)
```

```{r}
tabla_univariada(encuesta, p41f, desertor)
```
